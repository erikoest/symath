...
module SyMath
  ...
  def SyMath.atos(s)
    if s.is_a?(Array)
      return '['+ s.map { |e| SyMath.atos(e) }.join(', ') + ']'
    else
      return s.to_s
    end
  end

  describe SyMath::Poly::DUP do
    # Map expression to dup
    def dup(exp)
      return SyMath::Poly::DUP.new(exp)
    end
  ...
  describe SyMath::Poly::Galois do
    x = :x.to_m

    # Map array to galois poly
    def g(a, p)
      return SyMath::Poly::Galois.new({ :arr => a, :p => p, :var => :x.to_m })
    end
    ...
    def a(g)
      if g.is_a?(Array)
        return g.map { |e| a(e) }
      elsif g.is_a?(SyMath::Poly::Galois)
        return g.arr
      else
        return g
      end
    ...
    context 'Conversions' do
      dup = SyMath::Poly::DUP.new(x**4 + 7*x**2 + 2*x + 20)

      it "dup to gl" do
        gl = SyMath::Poly::Galois.new({ :dup => dup, :p => 5 })
